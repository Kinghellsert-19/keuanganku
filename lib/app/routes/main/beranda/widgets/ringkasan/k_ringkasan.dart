...
class Data {
  ...
  Future updateData() async {
    _listPemasukan = await SQLHelperIncome().readByWaktuAndSortir(
        waktuTransaksi, SortirTransaksi.Default,
        db: db.database);
    _listPengeluaran = await SQLHelperExpense().readByWaktuAndSortir(
        waktuTransaksi, SortirTransaksi.Default,
        db: db.database);
...
class KRingkasan extends StatefulWidget {
  const KRingkasan(
      {super.key, required this.callback, required this.widgetData});
...
class _KRingkasanState extends State<KRingkasan> {
  ...
  Widget buildBody(BuildContext context,
      {required double totalPemasukanX, required double totalPengeluaranX}) {
    return KCard(
      title: "Ringkasan",
      icon: icon,
  ...
  Widget build(BuildContext context) {
    return KFutureBuilder.build(
        ...
        whenError: makeCenterWithRow(
            child: const CircularProgressIndicator(
          color: Colors.white,
        )),
        whenWaiting:
            buildBody(context, totalPemasukanX: 0, totalPengeluaranX: 0),
